"""
本地图库个体查询网页（正式入口 4/7）：上传单张照片 → 检测裁剪 → r4 特征 →
全库 Top-K 检索 → 二态候选提示（known / unknown）。
...
gallery 与查询统一为 "YOLO 检测裁剪 + r4 微调特征"；查询模型自动
...
def main():
    ...
    parser.add_argument("--det-conf", type=float, default=det_cfg.get("conf", 0.25))
    parser.add_argument("--det-imgsz", type=int, default=det_cfg.get("imgsz", 1024))
    parser.add_argument("--det-device", default=det_cfg.get("device", "auto"),
