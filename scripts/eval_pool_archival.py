...
from scripts.extract_embeddings import extract_embeddings  # noqa: E402
...
def run():
    ...
    if not args.reuse_feats:
        ...
        extract_embeddings(rel_manifest(args.center_crops, center_ids, feat), args.center_crops,
                           args.model, feat / "center")
        # 散图：YOLO 裁剪池
        yolo_manifest = pd.read_csv(args.yolo_crops / "crops_manifest.csv")
        yolo_ids = sorted(yolo_manifest["image_id"].tolist())
        extract_embeddings(rel_manifest(args.yolo_crops, yolo_ids, feat), args.yolo_crops,
                           args.model, feat / "yolo")
        # gallery：pilot 整图（已确认个体，作口径 B 的检索目标）
        extract_embeddings(args.pilot, args.images_root, args.model, feat / "pilot_full")
