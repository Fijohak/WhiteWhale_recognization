...
流程（2026-08-17 工具链接入，E1/E2/E4 结论）：
散图（YOLO 检测裁剪，r3 特征，embeddings_pool_r3_yolocrop.npy 预提取）
→ 与同群已确认个体（r3 特征，embeddings_metric_r3_yolocrop.npy）余弦检索
→ Top-K 个体候选 + 分数。

语义：输出是 Candidate（候选划归），不代表自动确认；低分散图可能是
新个体候选，需人工审核。所有行保留 image_id / 原路径 / 群 / 候选分数。

阈值：默认 0.50 = E4 标定 FA≤5% 区间（0.5-0.6）的下限；散图归档场景的
正式标定待更多数据（本特征空间的分数分布与旧预训练特征不同）。
...
def load_gallery(emb_path: Path, meta_path: Path):
    """已确认个体的 r3+YOLO 裁剪特征（meta 含 confirmed_identity / 群）。"""
...
def eval_within_group(gallery: pd.DataFrame) -> dict:
    ...
    用同一批 r3+YOLO 裁剪特征，量化"限制同群"对检索的影响。
    """
    groups = {}
...
def main():
    ...
    parser.add_argument("--pool-embeddings", type=Path,
                        ...
                        help="散图 r3+YOLO 裁剪特征（scripts/extract_r3_yolocrop.py 预提取）")
    ...
    parser.add_argument("--threshold", type=float, default=0.50,
                        help="低于此分标记为 low_confidence（E4 标定 FA≤5% 区间下限；"
                             "散图归档正式标定待数据）")
    ...
    pool = load_pool(args.pool_embeddings, args.pool_meta)
