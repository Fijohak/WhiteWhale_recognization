...
> 执行状态（2026-08-31）：M1–M4 的领域闭环与 M5 的部署骨架已通过自动化验收；现有 r4/202 条 Gallery 已投影为可追溯、`provisional_unvalidated` 的初始生产模型与活动 Catalog。总览、批次、查询、Job/Artifact、模型评估/回滚、Worker、系统审计、部署失败原因、真实最小测试和离线包结构已补齐；完成性复核仍缺约 3000 张真实批次断网端到端验收、宿主机 systemd 正式安装、管理员在 Tailscale 控制台启用 Serve，以及外部尚未提供的最终审核 CSV/身份迁移表。因此本计划尚未完成。
...
- [x] **完成 Web/API 管理面与全量审计**：总览、批次、单图/整批查询、Job/Attempt/Artifact、模型评估/回滚、Worker、系统与不可变审计已完成；登录、下载、EXIF、查询、审核、发布、Catalog/Model 回滚和令牌撤销均有对应审计事件。
