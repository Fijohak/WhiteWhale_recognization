...
* **项目目标（两者并存）**：① **研究个体识别方法**——可靠的批内分组、跨时间匹配与新个体发现（开放集），方法经实验标定与验证（EXPERIMENT_LOG E1–E9）；② **落地为辅助归档工具**——输入一批新照片自动按个体分组、每簇取最清晰代表图归档，减少工作人员逐张手工整理的工作量（2026-08-17 数据提供方对齐时强调的应用诉求）；"新个体识别"不是独立目的，而是低置信度输出的自然副产品；
