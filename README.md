...
| 1.9 | 散图连拍整串划归：散图池按拍摄序列分组，任一帧匹配上某个体则整串划归候选（A9 抽样核验后执行；一次确认一串，大幅提升收集效率） | [ ] |
| 1.8 | 散图分拣（人工待办）：322 张散图（loose_known，9 批次）人工分配到对应个体文件夹 | [ ] |
...
| 3.6 | 历史库核验：20140806 43 组 202 张 Candidate 级标签人工核验（包已就绪：batches_history/history_verify.csv，202/202 带特征辅助） | [ ] |
| 3.7 | 6 张撤回照片多人复核投票重审（E9 撤回；关键判定须多人独立标注 + 投票裁决，提升审核精度与可信度） | [ ] |
| 3.8 | 跨年匹配重建（依赖 3.6 + 3.7 通过后） | [ ] |
...
| 4.3 | 背鳍轮廓特征（CurvRank 风格：缺口、凹陷、曲率）——**镜像应用**：左右剪影近似对称（A14），单侧个体可镜像检索跨侧同体；斑点/伤口等表面特征须忽略 | [ ] |
| 4.4 | 特征对比（外观/轮廓/融合） | [ ] |
| 4.5 | 特征可视化与错误分析 | [ ] |
...
| 6.11 | 审核网页支持多人复核投票：多审核人独立标注 + 汇总投票裁决（跨年匹配等关键判定多人把关，配合 3.7） | [ ] |
