# WhiteWhale_recognization
attempt to explore a method for individual identification of Indo-pacific humpback dolphins.
# 中华白海豚个体识别项目

## 1. 项目简介

本项目面向中华白海豚野外调查影像，目标是从野外调查照片中学习白海豚外观特征，并根据背鳍轮廓、缺口、伤痕和色斑等稳定特征，发现可能属于同一个体的影像记录。

当前数据集按**Anchor / Pool 语义**理解（详见第 5 节）：历史整理时工作人员从每个个体中挑选一张质量最高的代表照片（**Anchor**，位于 `80 and above` / `70-79` 下的数字子文件夹），其余照片未严格归档、混入公共照片池（**Unresolved Image Pool**，如 `70-79` 散图）。**目录结构不直接等于个体身份标签**。项目通过预训练特征提取、Anchor-based Top-K 检索与人工审核，逐步恢复可信个体目录；检索与聚类结果仅作为人工核验的候选。

---

## 2. 项目背景

中华白海豚个体识别通常依赖背鳍及其周围身体区域的可辨识特征，包括：

* 背鳍整体轮廓；
* 背鳍尖端形状；
* 背鳍后缘的缺口和损伤；
* 身体表面的疤痕、斑点和色素分布；
* 背鳍与身体连接区域的形态；
* 不同时间拍摄时仍然稳定存在的外观特征。

需要特别注意的是，**同一只白海豚的背鳍有左右两侧（两面）**：两侧的缺口、伤痕和色斑分布并不完全相同。识别时必须区分照片朝向——左侧照片与左侧照片直接比较、右侧照片与右侧照片直接比较；跨侧匹配只能作为弱证据，需要人工核验。

但当前数据中**每个个体基本只有一张照片**（少数个体含一次目击的连拍多帧，且已有分组为单张），尚无同一只海豚的左右两侧照片。这意味着：

* 目前不存在"同一体双侧"样本，训练时不必处理跨侧配对；
* 但每张照片的朝向（left / right / unknown）仍必须标注，防止未来补充数据时误把不同侧当作同一只来匹配；
* 单个个体仅一张图意味着训练类别样本数极少（每类 1～少数几张），模型需要利用跨个体的共性特征并依赖迁移学习。

当前数据主要由野外调查照片、视频、处理结果和人工整理文件组成。数据中可能包含日期、调查批次、摄影人员、连续拍摄序列、图像质量区间和候选关系等信息，但缺少可以直接用于监督分类训练的统一个体 ID。

因此，本项目将任务定义为：

> 在没有完整个体标签的条件下，学习白海豚背鳍图像的特征表示，并将可能属于同一个体的拍摄记录组织为候选分组。

---

## 3. 项目目标

### 3.1 主要目标

1. 整理原始数据集的目录结构和文件元数据；
2. 建立统一的图像索引和数据清单；
3. 从原始照片中提取适合个体识别的背鳍区域；
4. 使用预训练视觉模型提取图像特征，以代表照片（Anchor）为查询执行 Top-K 检索，发现同个体候选；
5. 根据特征相似度完成候选个体聚类；
6. 建立人工核验和结果修正流程；
7. 使用核验后的结果逐步优化特征模型；
8. 构建可扩展的白海豚个体检索系统。

### 3.2 非目标

项目第一阶段不追求：

* 完全自动确定所有白海豚的真实身份；
* 在没有人工核验的情况下直接生成最终个体数量；
* 仅根据文件夹编号跨调查推断个体身份；
* 直接使用原始整图完成可靠识别；
* 用模型结果完全替代领域专家判断。

---

## 4. 任务定义

本项目包含三个相互关联的任务。

### 4.1 图像特征学习

将每张背鳍图像转换为固定长度的特征向量。属于同一只白海豚的图像应当具有较高的特征相似度，不同个体的图像应当具有较低的特征相似度。

### 4.2 候选个体聚类

根据图像或拍摄序列的特征，将数据自动划分为若干候选簇。每个簇代表一个可能的白海豚个体。

聚类算法应允许：

* 自动发现候选簇数量；
* 将无法可靠归类的照片标记为噪声；
* 保留低置信度样本供人工检查；
* 支持后续合并和拆分候选簇。

### 4.3 相似个体检索

输入一张新的白海豚背鳍照片，系统返回特征最相似的若干候选个体，而不是强制输出唯一身份。

示例输出：

| 排名 | 候选个体          |  相似度 | 建议     |
| -- | ------------- | ---: | ------ |
| 1  | Candidate-017 | 0.91 | 建议重点核验 |
| 2  | Candidate-043 | 0.82 | 次要候选   |
| 3  | Candidate-006 | 0.75 | 相似度较低  |

当最高相似度低于设定阈值时，系统应将样本标记为“可能的新个体”或“需要人工判断”。

---

## 5. 数据集理解

当前数据集的实际结构（2026-08-07 扫描盘点；2026-08-19 命名规范升级）：

> **命名规范（2026-08-19 起）**：数据根目录名 = `YYYYMMDD NN`（年月日 + 空格 + 群体编号，编号仅当日唯一）。
> 旧数据 `01`/`03` 已改名为 `20140806 01`/`20140806 03`；新批次按此命名（如 `20140418 01`）。
> **新数据结构精简**：仅 `70-79` / `80 and above`（部分批次含 `nn relationship`，非必有；数据提供方要求忽略低分数据）；
> 旧批次后续同样裁剪为该结构。session 标识 = 目录名本身（脚本已去 01/03 硬编码）。

```text
I:\20140806 01（旧批次，改名自 01；调查 20140417W01，地点代码 SZi，批次 01）
├── 70-79/                      # 70-79 分组
│   ├── 05/ 11/ 12/ 13/ 14/     # 子文件夹 = 代表照片 Anchor（5 组，非全局 ID）
│   └── *.JPG                   # 59 张散图，Unresolved Image Pool（人工跟进中）
├── 80 and above/               # 80 分及以上
│   └── 01/ … 10/               # 子文件夹 = 代表照片 Anchor（10 组，28 张）
├── 50-59/ 60-69/ below 50/     # 低于 70 分，暂不用于训练（后续将裁剪移除）
├── MO/  miscellaneous/  nn relationship/   # 拍摄者 / 其他目录
├── 20140417W01.txt             # 调查记录
└── individual.docx             # 人工对照材料

I:\20140806 03（旧批次，改名自 03；调查 20140417W03，地点代码 HBi，批次 03）
├── 70-79/
│   ├── 21/ 22/ 23/ 26/ 28/ 29/ 30/   # 子文件夹 = 代表照片 Anchor（7 组，非全局 ID）
│   └── *.JPG                   # 148 张散图，Unresolved Image Pool（人工跟进中）
├── 80 and above/
│   └── 01/ … 31/               # 子文件夹 = 代表照片 Anchor（22 组，155 张）
├── DEREK/  MO/  RAY/  miscellaneous/  nn relationship/
├── 20140417W03.txt
└── individual.docx

I:\01.zip / I:\processed.zip / I:\Processed 2.zip   # 原始压缩包（只读）
```

### 5.1 数据规模（2026-08-07 扫描统计）

| 项 | 数量 |
|---|---|
| 总图片数 | 2863（01：818，03：2045） |
| 已分组个体（labeled） | 43 个体 / 199 张（01：14 个，03：29 个） |
| 70-79 散图（loose_known） | 207 张（01：59，03：148） |
| 低于 70 分 / 拍摄者目录（ignored） | 2457 张 |
| 可读图片 / 损坏 | 2863 / 0 |
| 精确重复组（SHA-256） | 1 组 2 张 |

> 个体数按标签 `{session}_{group_id}` 计，跨调查同名编号未合并。
> `labeled_individuals`：单图个体 15 个，多图个体 28 个，最大个体 27 张（连拍）。

### 5.2 已确认事实

* **评分区间 = 图片分级**：W01 txt 计数（50-59:14, 60-69:100, 70-79:64, 80+:28）与 `I:\01` 目录图片数逐项一致；
* **MO / RAY / DEREK = 拍摄者代码**：txt 人员计数明确；
* **文件名双模式**：RAY 拍摄 `0145_20140417_SZi_01_RAY_0632.JPG`（编号_日期_地点_批次_人员_连拍号）；MO 拍摄 `RES20001.JPG`（无日期字段）；
* **01 = W01（SZi），03 = W03（HBi）**，同一天两群；
* **01 与 03 是两个独立海豚群**：分组编号在群内各自独立（01_02 与 03_02 不是同一只），跨群照片默认视为不同个体，除非未来人工确认；
* **根目录命名 = `YYYYMMDD NN`**（2026-08-19 数据提供方规范）：年月日 + 空格 + 群体编号，编号仅当日唯一（跨日期可重名，session 标识 = 完整目录名）；
* **`nn relationship` 目录 = 疑似亲缘关系个体样本**（2026-08-19 数据提供方确认，**非每个批次必有**）：一图两鳍（同框多头），不参与个体分组，scan_dataset 标记 relation_note=nn_relationship，是多头同框检测（TASKS 6.10）的直接验证对象；亲缘关系仍需人工确认（同框 ≠ 有亲缘）；

### 5.3 待确认假设（A1–A10）

| # | 假设 | 状态 |
|---|---|---|
| A1 | 评分区间为质量/匹配置信度分级 | 基本证实（W01 计数一致） |
| A2 | MO/RAY/DEREK 为拍摄者代码 | 已证实 |
| A3 | SZi/HBi 为调查地点代码 | 推测 |
| A4 | 80 and above 数字分组为代表照片（Anchor）而非个体 ID | 已确认（历史整理方式） |
| A5 | `sj of 01` = 伴随/相关个体关系 | 推测 |
| A6 | `nn relationship` = 相邻关系候选组 | 推测（txt 字段为空） |
| A7 | processed / Processed 2 为不同批次 | 推测（SHA256 无重叠） |
| A8 | txt 计数为全量，目录为整理后子集 | 推测 |
| A9 | 文件名连拍号 = 连续拍摄序列 | 推测，须抽样核验后用于划分 |
| A10 | RES+数字（MO 拍摄）无连拍信息 | 推测 |
| A11 | 跨时间重复拍摄到同一群/同一只海豚（无全局编号） | **推定**（数据提供方，2026-08-17） |
| A12 | 个体特征跨时间会变化（成长、意外事件致疤痕/轮廓改变） | **推定**（鲸豚领域已知现象） |
| A13 | 批内散图池照片全部属于本批已确认个体（无新个体混入） | 批内既定；跨批不保证 |

> 所有假设在确认前：不得作为训练标签、不得作为数据划分依据、Manifest 中只能作为带 `_guess` 后缀的启发式字段。
> 推定（Assumed）与既定（Confirmed）必须区分记录：A11/A12 未经数据验证，不依赖其成立即可运行批内任务。

### 5.4 数据使用要点

* 当前只使用 01 与 03 两个目录中 `70-79`、`80 and above` 两个区间（≥70 分）的数据；
* 这两个区间下的**数字子文件夹 = 历史挑选的代表照片（Anchor）**，**不代表全局个体 ID**，可作 Anchor 检索的查询集，不可直接作监督标签；
* **每个 Anchor 基本只有一张照片**（极少数含连拍多帧），属于极端少样本的个体识别问题；
* `70-79` 散图（207 张）属于 **Unresolved Image Pool**：同调查内可能属于某个已选代表照片的个体，但具体归属未确认，只能作为检索 Gallery 的候选，不可作标签；
* 分组编号只在同一调查内有效，01 与 03 之间同名编号是否对应同一只海豚需人工核验；
* 01 与 03 是**同一天（20140417）拍摄的两群海豚**，属于两个独立调查批次，跨调查的个体对应关系尚未核验；
* 背鳍有左右两面，但当前每个个体只有一张照片、尚无双侧样本；照片仍须记录朝向（left / right / unknown），左右侧分别比较。

项目需要区分以下概念：

| 概念    | 说明                  |
| ----- | ------------------- |
| 调查批次  | 一次野外调查、日期或航次        |
| 拍摄序列  | 短时间内连续拍摄的一组照片       |
| Anchor（代表照片） | 历史整理中挑选的个体代表照片，作检索查询；未经确认前不代表身份 |
| Unresolved Image Pool | 未归属照片池，作检索 Gallery |
| 候选分组  | 原数据中人工或程序初步整理出的图片集合（Candidate Cluster） |
| 个体 ID | 经人工核验后确认的白海豚身份（Confirmed Individual） |
| 图像质量  | 图像清晰度、目标大小或人工评分     |
| 关系备注  | 文件夹名称中记录的候选关联信息     |

### 5.5 任务定位与数据语义（2026-08-17 与数据提供方对齐）

任务按时间尺度分层，批内与跨时间的语义、任务与可靠性不同：

| 尺度 | 数据语义 | 任务 | 可靠性 |
|---|---|---|---|
| 批内（同一天调查） | 每个已确认个体可视为独立；散图池照片**有主**（属于本批某已确认个体，A13） | 归档分配、池找回（封闭集） | 特征短期稳定，可靠 |
| 跨时间（不同日期批次） | 可能重复拍摄到同一群，个体被单独标记但无全局编号；跨时间同体对为**推定**（A11） | 与历史个体库匹配，发现疑似新个体（开放集） | 特征可能漂移（A12），结果仅为候选，须人工核验 |

要点：

* **项目最终目的：加速数据处理环节**（2026-08-17 项目负责人澄清）——输入一批新照片，自动识别同一个个体的图片（批内分组），每簇取最清晰的一张代表图归档，减少工作人员逐张手工整理的工作量；"新个体识别"不是目的，而是低置信度输出的自然副产品；
* **批内主路径 = 自动归档**：检测背鳍 → 裁剪 → embedding → 批内聚类（Candidate Cluster）→ 每簇选最清晰代表图 → 人工审核确认归档；检索用于"与历史库匹配"（这只见过没），聚类用于"这一批有几只"；
* 项目负责人期望：辨别新个体的出现（开放集）；跨时间 Re-ID（个体再识别）不作为当前目标；
* "新个体"判定依赖"认出旧个体"的能力：特征漂移的已知个体（成长、伤病致疤痕/轮廓改变）是最主要的"疑似新个体"假阳性来源，通过多特征通道互补 + 置信度输出 + 人工核验兜底；
* 跨时间匹配结果默认落在 possibly_same 关系，个体档案记录照片时间戳与特征变化历史，支持专家核验；
* 跨时间同体对会随批次积累逐步被发现（匹配 + 人工审核），是"新个体发现"阈值标定的依据；在积累之前，阈值保守、多报候选、人工兜底（宁可拆分不可错并）。

---

## 6. 数据使用原则

### 6.1 分层信任目录标签

* `80 and above` 与 `70-79` 中的数字子文件夹是**历史挑选的代表照片（Anchor）**，可作 Anchor-based 检索的查询集，但**未经人工核验前不代表个体身份**，不可作监督分类标签；
* 分组编号只在同一调查（01 或 03）内成立，跨调查同名编号不默认对应同一只海豚，需人工核验后才可作全局个体 ID；
* `70-79` 中未归入子文件夹的散图属 Unresolved Image Pool，仅作检索 Gallery，不作标签；
* 低于 70 分的区间以及 `MO`/`RAY`/`DEREK`/`miscellaneous`/`nn relationship` 等目录暂不用于个体标签。

### 6.2 避免重复数据

压缩文件和解压后的目录可能包含相同数据。数据索引阶段应进行文件哈希或图像相似度检查，避免重复统计和重复训练。

### 6.3 保留原始数据

原始数据目录保持只读。所有清洗、裁剪、重命名和整理结果保存在独立目录中，避免破坏原始材料。

### 6.4 保留数据来源信息

每张处理后的图片都应能够追溯到：

* 原始文件路径；
* 调查日期；
* 拍摄批次；
* 连续拍摄序列；
* 原始候选分组；
* 图像质量区间；
* 处理方式；
* 人工核验状态。

---

## 7. 数据处理流程

### 阶段一：数据盘点

对数据集进行整体统计，包括：

* 图片总数；
* 视频和非图片文件数量；
* 调查日期和批次数量；
* 文件格式和分辨率分布；
* 各目录中的图片数量；
* 重复文件数量；
* 无法读取或损坏的文件数量；
* 不同质量区间的数据量；
* 每个连续拍摄序列的图片数量。

输出统一的数据清单，例如：

| 字段            | 含义         |
| ------------- | ---------- |
| image_id      | 项目内部唯一图片编号 |
| original_path | 原始文件路径     |
| session_id    | 调查日期或调查批次  |
| sequence_id   | 连续拍摄序列编号   |
| source_group  | 原始候选分组     |
| quality_band  | 原始质量或评分区间  |
| side          | 左侧、右侧或未知   |
| crop_path     | 背鳍裁剪图路径    |
| review_status | 人工核验状态     |

### 阶段二：数据清洗

需要识别和处理：

* 无法打开的文件；
* 完全重复的图片；
* 高度相似的连续重复帧；
* 没有出现白海豚的图片；
* 背鳍不可见或目标过小的图片；
* 严重模糊或过度曝光的图片；
* 包含多只白海豚但无法确定目标的图片；
* 左右侧无法判断的图片。

低质量图片不一定永久删除，可以单独保留，等待后续模型能力提高后再次处理。

### 阶段三：背鳍区域提取

个体识别应主要基于背鳍及其附近身体区域，而不是整张海面照片。

第一阶段可以采用人工或半自动方式完成裁剪。裁剪区域应：

* 完整保留背鳍尖端；
* 完整保留背鳍前缘和后缘；
* 保留背鳍下方少量身体区域；
* 保留部分周围上下文；
* 尽量减少海面、船只和远处背景；
* 避免将背鳍边缘裁掉。

当项目积累足够的检测标注后，可以训练自动背鳍检测模型，替代人工裁剪。

### 阶段四：朝向识别

白海豚左右两侧的缺口、疤痕和色斑并不完全相同，因此需要记录图像朝向：

* left：展示个体左侧；
* right：展示个体右侧；
* frontal/rear：正面或背面角度；
* unknown：无法确认。

第一阶段建议分别对左侧和右侧照片进行特征比较，避免模型错误地将左右侧视为完全相同。

---

## 8. 模型方案

### 8.1 基线方案

首先使用已有预训练视觉模型提取背鳍特征，不对模型进行专门训练。

基线流程：

```text
背鳍裁剪图
→ 预训练视觉编码器
→ 特征向量
→ 特征标准化
→ 相似度计算
→ 聚类和检索
```

基线方案用于判断：

* 数据是否具有可区分的个体特征；
* 现有预训练模型是否能区分背鳍形态；
* 聚类结果是否主要受到背景和拍摄条件影响；
* 后续是否有必要进行自监督微调。

### 8.2 自监督学习

在没有个体标签的情况下，使用全部合格背鳍图片进行自监督训练。

模型通过同一张图片的不同增强视图学习稳定特征。增强方式应尽量模拟真实拍摄变化，同时避免破坏个体识别特征。

推荐增强：

* 轻微缩放和裁剪；
* 小角度旋转；
* 亮度和对比度变化；
* 轻微颜色变化；
* 轻微模糊；
* 轻微分辨率变化。

谨慎使用：

* 水平翻转；
* 大角度旋转；
* 大面积遮挡；
* 严重模糊；
* 会删除背鳍尖端或后缘的随机裁剪；
* 会改变背鳍轮廓的几何形变。

### 8.3 序列级特征

连续拍摄的多张图片通常属于同一次目击记录。直接对单张图片聚类，模型可能只学会识别连续帧，而无法完成跨日期识别。

因此需要建立序列级表示：

```text
连续拍摄序列
→ 每张图片提取特征
→ 排除低质量帧
→ 特征平均或加权融合
→ 得到序列级特征
```

最终优先比较不同日期或不同调查批次之间的序列特征。

### 8.4 无监督聚类

聚类阶段不预先假设白海豚个体数量。

聚类结果应包含：

* 高置信度候选簇；
* 低置信度边界样本；
* 无法归类的噪声样本；
* 簇间相似度；
* 每个簇的代表图片；
* 每个簇包含的调查日期和拍摄序列。

聚类算法的具体选择将在实验阶段比较，重点考察：

* 是否需要预先指定簇数量；
* 对异常样本的处理能力；
* 对参数变化的稳定性；
* 是否容易出现过度合并；
* 是否容易将同一个体拆成多个簇。

---

## 9. 人工核验流程

模型聚类结果不直接作为最终个体身份，需要经过人工核验。

人工核验应重点检查：

* 背鳍轮廓是否一致；
* 缺口的位置、大小和形状是否一致；
* 伤痕和色斑是否一致；
* 图像是否展示同一侧身体；
* 是否存在明显不同的背鳍特征；
* 是否因背景、日期或光线相似而被错误聚类；
* 是否存在两个候选簇实际属于同一个体。

核验操作包括：

* 确认候选簇；
* 拆分混合簇；
* 合并重复簇；
* 标记无法确定的样本；
* 标记疑似新个体；
* 记录判断依据；
* 记录核验人员和核验时间。

建议每次人工修改均保留版本记录，避免覆盖历史判断。

---

## 10. 伪标签迭代

人工确认后的候选簇可以作为伪标签，用于进一步训练个体特征模型。

迭代流程：

```text
预训练特征
→ 第一次聚类
→ 人工核验
→ 生成伪标签
→ 特征模型微调
→ 重新提取特征
→ 重新聚类
→ 再次人工核验
```

随着可靠伪标签数量增加，可以逐步从完全无监督方法过渡到弱监督或半监督方法。

高置信度样本优先用于训练，低置信度样本继续保留在待核验集合中。

### 10.1 第一轮伪标签训练（2026-08-14，已完成）

使用人工初审确认的 135 张 / 31 个体作为伪标签（非专家复核，结果叫 Candidate 特征），
ArcFace 度量学习（s=32, m=0.3）两阶段微调 MegaDescriptor-T-224：

| 指标 | 预训练基线 | 微调后 | 说明 |
|---|---|---|---|
| B 协议 R@1 / mAP | 0.239 / 0.332 | **0.473 / 0.541** | leave-one-out 全库检索 |
| A 协议 R@1 / mAP | 0.122 / 0.244 | **0.327 / 0.435** | 代表图查询（真实场景） |
| 独立验证个体 R@1 | 0.529 | **0.765** | 6 个体 17 张，模型未见 |
| 跨群误配（A 协议） | 19 次（12.2%） | 21 次（13.5%） | 01↔03 仍是最难区分项 |

置信度变化（见 `outputs/reports/confidence_check/summary.txt`）：跨群对相似度
p95 从 0.843 压到 0.465（≥0.70 的跨群对从 24.1% → 1.1%）；阈值判定精确率
≥0.70 时从 5.6% → 19.9%。分数从"几乎无信息"变为有筛选价值，但仍不足以
自动确认个体（同群不同个体对仍难分），人工核验环节保留。

微调特征：`outputs/embeddings/embeddings_metric.npy`（配套 meta + config）；
聚类：`outputs/clusters_metric/`（8 簇 + 61% 噪声，比预训练特征的大混簇更纯）。

### 10.2 同群散图划分（2026-08-14，已完成）

场景决策（用户确认）：当前优先做**同群内散图划分**（把 70-79 公共池中
207 张未归属散图划归到同群已确认个体），不做跨群全库检索；模型微调 +
跨群 hard negative 作为后期升级项（TASKS.md 5.6）。

流程与结果（`scripts/assign_pool.py`，`outputs/pool_assignment/`）：

- 散图（01: 59 / 03: 148）中心裁剪 0.55 → 预训练 MegaDescriptor 特征；
  gallery = 同群已确认个体照片（133 张 / 31 个体，裁剪特征）。
- 输出 `pool_candidates.csv`：每张散图的群内 Top-5 候选（个体 + 分数）、
  top1、低分（<0.55）标记为 `low_confidence_new_candidate`（疑似新个体）。
- 结果：199 张有候选，5 张原图缺失（01 群，原始数据问题，仅记录），
  3 张低分疑似新个体；Top1 分数 p50=0.825。
- 群内 leave-one-out R@1（`within_group_eval.json`）：01 群 0.474 / 03 群
  0.570 / 全库 0.549 —— 限制同群与全库相当，说明两群背鳍特征本来就相似，
  Top-1 正确率约 5 成，候选表必须人工审核后使用。

**人工反馈与发现（2026-08-14，`outputs/pool_assignment/pool_reviews.csv`）**：

- 3 张低分散图中：`0256`（01 群）为**距离太远**（背鳍约占画面 10% 且偏左），
  中心裁剪特征不可用，非新个体，留待人工裁剪 / 背鳍检测（TASKS 6.1）；
- `0292 / 0293`（03 群连拍）人工确认为**同一个体 07 号**，模型 Top1 均误判为
  09 号 —— 具体案例证实 09 / 07 个体特征接近，同群 Top-1 候选里可能系统性
  混入相似个体，审核散图时对"高置信却可疑"的候选应核对多张。
- 注：模型无多模态能力，以上为人工审核结论，记入 `pool_reviews.csv`
  （reviewer + 时间，可追溯）。

**工具链接入（2026-08-17，TASKS 6.9）**：`assign_pool.py` 与 `query_app.py`
改用 r3 跨群 HN 微调特征 + YOLO 检测裁剪（实验 E1/E2/E4 结论的落地）：
散图特征预提取于 `outputs/embeddings/embeddings_pool_r3_yolocrop.npy`，
gallery 为 `embeddings_metric_r3_yolocrop.npy`（`scripts/extract_r3_yolocrop.py`）。
群内 leave-one-out R@1 升至 01 群 0.842 / 03 群 0.912（旧预训练特征 0.474/0.570）。
注意：r3 特征分数空间整体下移（同体分数中位约 0.5，E3 揭示），
低分疑似新个体占比高于旧链路属特征现状而非故障，阈值（默认 0.50）待数据标定。

---

## 11. 评价方案

虽然主要训练过程是无监督的，但仍需要建立少量人工确认的评估集。

评估集应尽量包含：

* 多个已确认个体；
* 每个个体的多个拍摄序列；
* 不同调查日期；
* 不同拍摄角度；
* 不同光照和海况；
* 左侧和右侧照片；
* 不同图像质量等级。

### 11.1 检索评价

用于判断输入一张照片后，正确个体是否出现在候选列表中。

主要指标：

* Rank-1；
* Rank-5；
* Mean Average Precision；
* 同个体和不同个体相似度分布。

### 11.2 聚类评价

在人工确认子集上评价：

* 聚类纯度；
* 调整兰德指数；
* 标准化互信息；
* 同一个体被拆分的比例；
* 不同个体被错误合并的比例；
* 噪声样本比例。

### 11.3 开放集评价

系统需要判断新照片是否可能属于数据库之外的新个体。

重点关注：

* 已知个体误判为未知的比例；
* 未知个体错误匹配到已知个体的比例；
* 不同相似度阈值下的精确率和召回率；
* 高风险错误合并数量。

在保护动物个体识别中，将两个不同个体错误合并可能直接影响种群数量估计，因此需要优先控制错误合并风险。

---

## 12. 实验阶段规划

### 第一阶段：数据理解与最小基线

目标：

* 完成原始数据盘点；
* 建立统一数据清单；
* 选择一批清晰背鳍照片；
* 完成人工裁剪和朝向标记；
* 使用预训练模型提取特征；
* 完成第一次相似度检索和聚类。

建议数据规模：

* 300～1000 张清晰照片；
* 尽量覆盖多个调查日期；
* 保留连续拍摄序列信息。

主要交付物：

* 数据清单；
* 背鳍裁剪数据；
* 图像和序列特征；
* 候选聚类结果；
* 每个簇的可视化拼图；
* 基线实验报告。

### 第二阶段：自监督特征优化

目标：

* 使用全部合格背鳍图片进行自监督微调；
* 比较微调前后的检索和聚类结果；
* 分析模型是否主要关注背鳍轮廓；
* 优化数据增强和图像裁剪策略。

主要交付物：

* 自监督特征模型；
* 特征可视化结果；
* 聚类参数对比；
* 错误案例分析。

### 第三阶段：人工核验与伪标签训练

目标：

* 建立人工核验工作流；
* 确认一批高可信个体；
* 生成第一版伪标签；
* 使用伪标签进行度量学习或对比学习；
* 提升跨日期个体检索能力。

主要交付物：

* 候选个体数据库；
* 人工核验记录；
* 伪标签版本；
* 优化后的特征模型；
* 检索评价报告。

### 第四阶段：自动化系统

目标：

* 训练背鳍检测模型；
* 自动完成目标裁剪；
* 自动提取特征；
* 自动返回候选个体；
* 支持新个体标记和人工确认；
* 建立可持续更新的个体数据库。

主要交付物：

* 背鳍检测模块；
* 个体特征提取模块；
* 候选检索模块；
* 人工核验界面；
* 数据库更新流程；
* 完整实验文档。

---

## 13. 建议的项目目录

```text
chinese-white-dolphin-reid/
├── README.md
├── docs/
│   ├── dataset-description.md
│   ├── annotation-guide.md
│   ├── experiment-plan.md
│   └── review-guide.md
│
├── data/
│   ├── raw/                 # 原始数据，只读
│   ├── manifests/           # 数据清单
│   ├── cleaned/             # 清洗后的图片
│   ├── crops/               # 背鳍裁剪图
│   ├── reviewed/            # 人工核验结果
│   └── splits/              # 训练与评估划分
│
├── outputs/
│   ├── embeddings/          # 图像和序列特征
│   ├── clusters/            # 聚类结果
│   ├── contact_sheets/      # 候选簇拼图
│   ├── retrieval/           # 检索结果
│   ├── visualizations/      # 特征可视化
│   └── reports/             # 实验报告
│
├── models/
│   ├── pretrained/
│   ├── self_supervised/
│   └── metric_learning/
│
├── configs/
└── src/
```

原始数据不建议直接提交到公共代码仓库。仓库中只保存目录说明、数据清单模板、处理流程和必要的示例数据。

---

## 14. 风险与应对措施

### 14.1 原始候选分组并非真实身份

应对方式：

* 将其作为弱标签或参考信息；
* 不作为最终训练标签；
* 通过跨日期匹配和人工核验确认。

### 14.2 模型根据背景而非背鳍聚类

应对方式：

* 使用背鳍裁剪图；
* 减少海面背景；
* 使用跨日期评估；
* 检查模型关注区域；
* 比较同背景不同个体的特征。

### 14.3 连续帧造成数据泄漏

应对方式：

* 将连续拍摄序列视为不可拆分的数据单元；
* 训练集和测试集按序列或日期划分；
* 不允许同一连拍组同时出现在训练和测试中。

### 14.4 左右侧照片错误匹配

应对方式：

* 记录图像朝向；
* 分别建立左右侧特征；
* 不默认使用水平翻转；
* 对跨侧匹配设置更严格的人工核验规则。

### 14.5 聚类结果过度合并

应对方式：

* 使用保守的相似度阈值；
* 优先保证候选簇纯度；
* 将不确定样本标记为噪声；
* 对高风险合并进行人工复核。

### 14.6 聚类结果过度拆分

应对方式：

* 比较不同日期的序列级特征；
* 使用多张图片构建个体代表特征；
* 在人工核验阶段支持候选簇合并；
* 使用确认后的伪标签继续优化模型。

### 14.7 个体外观随时间变化

伤痕、缺口和色斑可能随时间发生变化。系统应允许同一个体在不同时间表现出一定差异，并保留日期信息用于长期分析。

---

## 15. 数据伦理与保护要求

中华白海豚属于保护动物，项目应遵循数据提供方、研究机构和相关保护法规的要求。

需要注意：

* 不公开敏感的调查地点和精确坐标；
* 不公开未经授权的原始调查影像；
* 不将模型用于干扰、追踪或伤害动物；
* 对外发布数据前进行权限和隐私检查；
* 明确模型结果仅作为研究辅助；
* 保留人工审核环节；
* 对错误识别可能造成的种群统计影响进行说明；
* 在论文和项目中正确标注数据来源。

---

## 16. 预期成果

项目完成后，预期形成以下成果：

1. 统一的数据集说明和数据清单；
2. 清洗后的白海豚背鳍图像数据；
3. 图像级和序列级特征库；
4. 候选白海豚个体聚类结果；
5. 可供人工核验的候选簇拼图；
6. 白海豚个体相似度检索模型；
7. 已确认个体及其历史拍摄记录数据库；
8. 新个体发现和人工确认流程；
9. 模型评价和错误案例分析报告；
10. 可持续更新的数据处理与模型训练流程。

---

## 17. 项目成功标准

第一阶段成功不以“完全识别全部白海豚”为标准，而以以下结果为标准：

* 数据结构和字段含义得到基本梳理；
* 每张处理图片能够追溯到原始文件；
* 模型能够提取稳定的背鳍特征；
* 同一拍摄序列的图片通常具有较高相似度；
* 部分跨日期的同一个体可以被成功检索；
* 聚类结果明显优于随机分组；
* 模型没有主要按照背景、日期或相机进行聚类；
* 人工能够高效检查和修正候选簇；
* 不确定样本能够被保守地标记为噪声；
* 项目形成可重复、可扩展和可审计的工作流程。

---

## 18. 当前待确认事项

在正式开展训练前，需要继续确认：

* `70-79` 组散图的归属（01：59 张、03：148 张，人工跟进中）；
* 01 与 03 之间同名编号分组是否对应同一只海豚（跨调查个体对应）——**已确认：两个群编号独立，跨群默认不同个体**；
* 每张照片的背鳍朝向（左侧 / 右侧 / 未知）标注；
* `50-59`、`60-69`、`below 50` 区间的含义与是否可用于训练；
* `MO`、`RAY`、`DEREK` 等代码对应的人员；
* `nn relationship` 和 `sj of 01` 等关系说明；
* `individual.docx` 是否包含人工个体对照结果；
* 数据是否允许用于模型训练和公开研究；
* 是否有领域专家能够参与个体核验。

---

## 19. 方法概述

本项目的核心方法可以概括为：

```text
原始野外影像
→ 数据清洗与元数据整理
→ 代表照片 Anchor / 照片池 Pool 识别
→ 朝向识别（左右侧）
→ 预训练模型提取 embedding（zero-shot，不训练）
→ Anchor-based Top-K 检索（query=Anchor, gallery=Pool）
→ 人工审核 same / different / uncertain
→ 扩充 Confirmed Individual
→ 使用多个确认样本再次检索（prototype / 融合排名）
→ 逐步恢复个体目录（Confirmed Individual Catalogue）
→ 建立可靠训练 / 测试数据
→ 后期再进行 Metric Learning / Open-set Re-ID
```

项目最终定位不是完全自动化的身份判定工具，而是一个辅助研究人员整理照片、发现候选匹配、减少人工检索工作量的白海豚个体识别系统。

---

## 20. 离线查询客户端

本地 Web 小工具（`src/query_app.py` + `src/query_app.html`）：上传一张背鳍照片，对全部 gallery 特征做 Top-K 检索，输出三态判定。

### 20.1 功能

* 上传照片 → YOLO 背鳍检测裁剪（未检出回退整图）→ r3 跨群 HN 微调特征 → 全库 Top-K 检索（2026-08-17 工具链接入：散图场景检测裁剪显著更优 E2、特写图打平 E1，统一链路避免混合语义）；
* 三态判定（固定阈值，默认 0.55 = E4 标定 FA≤5% 区间 0.5-0.6 中值）：
  * `known`：最高相似度 ≥ 阈值，展示 Top-K 候选（仍需人工核验）；
  * `unknown`：最高相似度 < 阈值，提示"疑似未知个体（可能新个体）"，仍返回 Top-K 供参考；
* 候选展示：缩略图 + 相似度 + 来源（source_group / session / quality / cluster）+ 本次查询裁剪信息（detect/fallback）；
* 所有候选一律标注"待人工核验"，不自动判定身份；
* 模型匹配防护：查询模型必须与 gallery 特征同模型（MegaDescriptor 与 DINOv2 均为 768D 但分布不同，不可直接比较）。

### 20.2 用法

```bash
pip install fastapi uvicorn python-multipart
python src/query_app.py --port 8000
# 浏览器打开 http://127.0.0.1:8000
```

也可**双击仓库根目录 `start_query_app.bat`** 一键启动：后台起服务（最小化窗口）→ 端口就绪后自动打开浏览器。

参数：

| 参数 | 默认值 | 说明 |
|---|---|---|
| `--embeddings` / `--meta` | `outputs/embeddings/embeddings_metric_r3_yolocrop.npy`（+meta） | gallery 特征（r3 微调 + YOLO 裁剪，`scripts/extract_r3_yolocrop.py` 提取） |
| `--pilot` | `outputs/pilot/pilot_set.csv` | 追溯字段 |
| `--images-root` | `I:/` | 源图根目录（候选缩略图用） |
| `--model` | 自动 | 查询模型；默认按 gallery 特征自动匹配，显式指定时校验必须同源 |
| `--metric-ckpt` | `outputs/metric_learning/r3/best.pt` | 微调权重（gallery 为微调特征时使用） |
| `--dinov2-weight` | 无 | DINOv2 官方权重 .pth（gallery 为 dinov2 特征时必填） |
| `--threshold` | `0.55` | known / unknown 判定阈值（E4 标定 FA≤5% 区间 0.5-0.6 中值） |
| `--detect` / `--no-detect` | 开 | 查询图先走 YOLO 背鳍检测裁剪（默认开；未检出回退整图） |
| `--k` | `10` | 返回候选数 |
| `--host` / `--port` | `127.0.0.1` / `8000` | 监听地址 |

> 注意：gallery 特征必须与查询模型同源。默认 r3+YOLO 裁剪特征需配 r3 微调权重（已自动处理）；
> 切换回预训练特征时用 `--embeddings outputs/embeddings/embeddings.npy --meta outputs/embeddings/embeddings_meta.csv`，
> 模型会自动切回预训练 MegaDescriptor。

### 20.3 语义约束

与项目其余部分一致：检索结果是 Candidate（候选），人工核验后才能称为个体；`unknown` 分支提示的是"疑似新个体"，不自动入库；所有候选保留 image_id / source_group / session / quality 追溯字段。
