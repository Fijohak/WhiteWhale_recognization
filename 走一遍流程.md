...
| `outputs/metric_learning/r3/best.pt` | ✅ 已有 | r3 特征模型（正式特征源） |
...
| `outputs/embeddings/embeddings_metric_r3_yolocrop_v2.*` | ❌ 需生成 | 历史库特征（第 3 步，要用你的 r3 权重 + YOLO 现跑） |
...
- 做三件事：历史库（20140806 01/03 的 202 张 labeled）→ **YOLO 检测裁剪**（`models/detectors/yolov8n_dorsalfin.pt`）→ **r3 特征**（`outputs/metric_learning/r3/best.pt`）
- 产出 `outputs/embeddings/embeddings_metric_r3_yolocrop_v2.npy` + `_meta.csv`，以及 `outputs/crops_yolo_gallery/` 裁剪图
...
- ⚠️ **注意坑**：`launch_query.py` 默认找的是 `embeddings_metric_r3_yolocrop.npy`（**不带 v2**），而第 2 步生成的是 **v2** 版本。二选一解决：
  - **复制一份**（最简单）：
    ```bash
    cp outputs/embeddings/embeddings_metric_r3_yolocrop_v2.npy outputs/embeddings/embeddings_metric_r3_yolocrop.npy
    cp outputs/embeddings/embeddings_metric_r3_yolocrop_v2_meta.csv outputs/embeddings/embeddings_metric_r3_yolocrop_meta.csv
    ```
  - 或启动时显式指定 `--embeddings ...v2.npy --meta ...v2_meta.csv`
