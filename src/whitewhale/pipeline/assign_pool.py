...
流程（E1/E2/E4 结论）：散图（YOLO 检测裁剪，r3 特征，
embeddings_pool_r3_yolocrop.npy 预提取）→ 与同群已确认个体（r3 特征，
embeddings_metric_r3_yolocrop.npy）余弦检索 → Top-K 个体候选 + 分数。

语义：输出是 Candidate（候选划归），不代表自动确认；低分散图可能是
新个体候选，需人工审核。所有行保留 image_id / 原路径 / 群 / 候选分数。

阈值：默认 0.50 = E4 标定 FA≤5% 区间（0.5-0.6）的下限。
